		...
		<?php if (isset($filter['filters'])) { ?>
            ...
            <?php if ($filter['style_id'] == 'checkbox') { ?>
                <div class="filter-item filter-item-checkbox">
                    <b class="filterline"><?php echo $filter['name']; ?></b>
                    <ul class="filterline">
                    <?php foreach ($filter['filters'] as $filter_value) { ?>
						<?php if ($filter_value['count'] || !$count_enabled) { ?>
						...
						<?php } else { ?>
							<li><input type="checkbox" disabled="disabled"><span><?php echo $filter_value['name']; ?> <?php echo $filter_value['view_count']; ?></span></li>
						<?php } ?>
                    <?php } ?>
                    </ul>
                </div>
			...
			<?php if ($filter['style_id'] == 'slider') { ?>
                <div class="filter-item filter-item-slider">
					...
					<input type="text" id="price" class="filter_active" data-key="p" data-value="<?php echo $filter['filters'][0]['value'] . ',' . $filter['filters'][1]['value']; ?>" readonly />
...
<script>
	...
	function addButtonReset() {
		var href = location.href;
		if (/(\?|&)filter=(.*?)/.test(href)) {
			$("#filter_apply_button").after('&emsp; <a style="float: right" class="button" onclick="resetFilter();"><?php echo $text_reset_filter; ?></a> ');
		}
	}
