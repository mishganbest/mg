<?php  
class ControllerModuleRecall extends Controller {
	private $json = array();

	protected function index() {
		$this->language->load('module/recall');
    	...
    	$this->data['text_recall'] = $this->language->get('text_recall');
	...
	$this->data['show_product'] = $this->config->get('show_product');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/recall.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/module/recall.tpl';
		} else {
			$this->template = 'default/template/module/recall.tpl';
		}
	...
	public function ajax() {
		...
		if (count($errors)) {
			...
			if ($this->config->get('show_name') && !empty($this->request->post['user_name'])) {
			...
			if ($this->config->get('show_time')) {
				...
				$message .= $recommend_to_call . html_entity_decode($this->request->post['recommend_to_call'], ENT_QUOTES, 'UTF-8') . "\r\n";
			}
			if (!empty($this->request->post['product'])) {
				$product = $this->language->get('text_product_name');
				$message .= $product . html_entity_decode($this->request->post['product'], ENT_QUOTES, 'UTF-8') . "\r\n";
			}
			if ($this->config->get('show_comment') && !empty($this->request->post['user_comment'])) {
			...
			if ($this->config->get('sendsms')) {
				$str = $this->request->post['user_phone'] . "\r\n";
				if ($this->config->get('show_name')) {
					$str .= $this->request->post['user_name'] . "\r\n";
				}
				if ($this->config->get('show_comment')) {
					$str .= $this->request->post['user_comment'];
				}
				$smslengh = (int)$this->config->get('smslength');
				if ($smslengh && strlen($str) > $smslengh) {
					$str = substr($str, 0, $smslengh);
				}
				$options = array(
	...
	private function validate($data) {
		...
		if ($this->config->get('show_email') && $this->config->get('required_email')) {
			...
			if (!preg_match($pattern, $data['user_email'])) {
				$errors['error_message'] = $this->language->get("error_email");
			}
		}
		if ($this->config->get('show_name') && $this->config->get('required_name')) {
			if (!isset($data['user_name']) || utf8_strlen($data['user_name']) < 1 || utf8_strlen($data['user_name']) > 32) {
				$errors['error_message'] = $this->language->get("error_user_name");
			}
		}
		if ($this->config->get('show_time') && $this->config->get('required_time')) {
			if (!isset($data['recommend_to_call']) || utf8_strlen($data['recommend_to_call']) < 1 || utf8_strlen($data['recommend_to_call']) > 32) {
				$errors['error_message'] = $this->language->get("error_recommend_to_call");
			}
		}
		if ($this->config->get('show_comment') && $this->config->get('required_comment')) {
			if (!isset($data['user_comment']) || utf8_strlen($data['user_comment']) < 1 || utf8_strlen($data['user_comment']) > 128) {
				$errors['error_message'] = $this->language->get("error_user_comment");
			}
		}
		if ($this->config->get('required_phone')) {
			$pattern = '/^[0-9]{4,11}$/i';
			if (!preg_match($pattern, $data['user_phone'])) {
				$errors['error_message'] = $this->language->get("error_user_phone");
			}
		}
		return $errors;
	}
