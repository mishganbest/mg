...
class ControllerPaymentAuthorizeNetAimSimple extends Controller {
    protected function index() {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/authorizenet_aim_simple.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/authorizenet_aim_simple.tpl';
        } else {
            $this->template = 'default/template/payment/authorizenet_aim_simple.tpl';
        }    
    ...
    private function get_value($name) {
        ...
        if (!empty($this->session->data['authorizenet_aim_simple'][$name])) {
            $this->data[$name] = $this->session->data['authorizenet_aim_simple'][$name];
        }
    }
    ...
    public function send() {
        ...
        if (curl_error($curl)) {
            ...
            if ($response_info[1] == '1') {
                if (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('authorizenet_aim_simple_hash') . $response_info[6] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false)))) {
                    ...
                    $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('authorizenet_aim_simple_order_status_id'), $message, false);                
                }
                
                unset($this->session->data['authorizenet_aim_simple']);
