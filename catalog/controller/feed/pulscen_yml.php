...
class ControllerFeedPulscenYml extends Controller {
	private $shop = array();
...
//				$data['cbid'] = 15;
				...
				$data['url'] .= '?utm_source=marketplace&utm_medium=pulscen';
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				...
				if ($this->config->get('yandex_yml_attr_vs_description')) {
					...
					$data['announce'] = $product['short_pulscen'];
					if ($product['pulscen_description']) {
					$data['description'] = $product['pulscen_description'];
					} else {
	...
	public function cmpProblem($aPr, $bPr) {
		if ($aPr['price'] == $bPr['price']) return 0;
		return $aPr['price'] < $bPr['price'] ? -1 : 1;
	}
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		uasort($soptions, array($this,'cmpProblem'));
		$firstOption = current($soptions);
		$sizePrice = '';
		$sizePriceAnnounce = '';
		$data['price'] = $firstOption['price'];
				foreach ($soptions as $option) {
					$size_option_name = $option['option_name'];
					$size_option_unit = $this->size_units[$option['option_id']];				
					$sizePrice.= "\n" .$option['name']. ' см - ' . money_format('%i', $option['price']) . ' руб.';
					$sizePriceAnnounce.= $option['name']. ' см - ' . money_format('%i', $option['price']) . ' руб.; ';
				}
		$data['announce'] = $sizePriceAnnounce . $data['announce'];
		$data['description'].= $sizePrice;
		$this->setOffer($data);
		return true;
	...
	*/
	...
	private function setOffer($data) {
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'announce'=>0, 'description'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'downloadable'=>0, 'adult'=>0, 'barcode'=>0));
