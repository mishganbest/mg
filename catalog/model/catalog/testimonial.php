...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data, $status) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET status = '".$status."', rating = '".$this->db->escape($data['rating'])."', name='".$this->db->escape($data['name'])."', city = '".$this->db->escape($data['city'])."', phone = '".$this->db->escape($data['phone'])."', email='".$this->db->escape($data['email'])."', vk = '".$this->db->escape($data['vk'])."', odnoklass = '".$this->db->escape($data['odnoklass'])."', image = '".$this->db->escape($data['image'])."', date_added = NOW()");
