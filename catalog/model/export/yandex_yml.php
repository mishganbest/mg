...
class ModelExportYandexYml extends Model {
	...
	public function getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required = true, $allowed_manufacturers = '') {
		...
		$query = $this->db->query("SELECT
			p.*, pd.name, pd.short_pulscen, pd.pulscen_description, pd.blizko_description, pd.description, pd.meta_description, m.name AS manufacturer, p2c.category_id, IFNULL(ps.price, p.price) AS price, wcd.unit AS weight_unit
