...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	private function getList() {				
		...
		$this->data['blank'] = $this->url->link('catalog/product/blank', 'token=' . $this->session->data['token'], 'SSL');
...
'video'      => $result['video'],
		...
		$this->data['button_blank'] = $this->language->get('button_blank');
  	...
  	private function getForm() {
		...
		$this->data['entry_short_pulscen'] = $this->language->get('entry_short_pulscen');
		$this->data['entry_pulscen_description'] = $this->language->get('entry_pulscen_description');
		$this->data['entry_blizko_description'] = $this->language->get('entry_blizko_description');
		...
		$this->data['entry_rate_stock'] = $this->language->get('entry_rate_stock');
	...
	$this->data['entry_option_sku'] = $this->language->get('entry_option_sku');
	$this->data['entry_rate_stock'] = $this->language->get('entry_rate_stock');
...
if (isset($this->error['video'])) {
		...
		if (isset($this->request->post['rate_stock'])) {
      		$this->data['rate_stock'] = $this->request->post['rate_stock'];
    			} elseif (!empty($product_info)) {
      		$this->data['rate_stock'] = $product_info['rate_stock'];
    			} else {
			$this->data['rate_stock'] = '';
		}
...
if (!empty($this->request->post['video']) && preg_match("/http:/i", $this->request->post['video'])) { 
	...
	public function blank() {
		...
		$this->data['column_product_rate'] = $this->language->get('column_product_rate');
		...
		$products = $this->model_catalog_product->getProductsBlank();

		foreach ($products as $product) {

			if ($product['product_id']) {

			$result = $this->model_catalog_product->getProductsRate($product['product_id']);

			$option_data = array();

			$options = $this->model_catalog_product->getProductOptionsBlank($product['product_id']);
			...
			$this->data['products'][] = array(
					...
					'sku'       => 	$result['sku'],
					'quantity'       => $result['quantity'],
					'rate_stock'       => $result['rate_stock']
				);
			
			}
		}

		$this->template = 'catalog/products_blank.tpl';
