...
class ControllerModuleRecall extends Controller {
	...
	public function index() {   
		$this->load->language('module/recall');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('recall', $this->request->post);		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/recall', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/recall', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['modules'] = array();
		
		if (isset($this->request->post['recall_module'])) {
			$this->data['modules'] = $this->request->post['recall_module'];
		} elseif ($this->config->get('recall_module')) { 
			$this->data['modules'] = $this->config->get('recall_module');
		}	
		...
		$this->template = 'module/recall.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
	...
	public function install(){
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('recall', array('smslength' => 70, 'sendsms' => false, 
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/recall')) {
