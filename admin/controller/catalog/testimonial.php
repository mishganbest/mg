...
class ControllerCatalogTestimonial extends Controller { 
	...
	private function getList() {
		...
		$this->data['sort_city'] = $this->url->link('catalog/testimonial', 'token=' . $this->session->data['token'] . '&sort=t.city' . $url, 'SSL');
	...
	private function getForm($is_edit) {
		...
		$this->data['entry_audio'] = $this->language->get('entry_audio');
		...
		$this->data['text_audiofile'] = $this->language->get('text_audiofile');
		$this->data['text_file_clear'] = $this->language->get('text_file_clear');
		...
		if (isset($this->request->post['vk'])) {
			...
			$this->data['vk'] = $testimonial_info['vk'];
		} else {
			$this->data['vk'] = '';
		}
		...
		if (isset($this->request->post['odnoklass'])) {
			...
			$this->data['odnoklass'] = $testimonial_info['odnoklass'];
		} else {
			$this->data['odnoklass'] = '';
		}
		...
		if (isset($this->request->post['image'])) {
			...
			$this->data['image'] = $this->model_tool_image->resize($testimonial_info['image'], 100, 100);
		} else {
			$this->data['image'] = '';
		}
		
		if (isset($this->request->post['image'])) {
			$this->data['s_image'] = $this->request->post['image'];
		} elseif (isset($testimonial_info)) {
			$this->data['s_image'] = $testimonial_info['image'];
		} else {
			$this->data['s_image'] = '';
		}
		...
		if (isset($this->request->post['audio'])) {
			$this->data['audio'] = $this->request->post['audio'];
		} elseif (isset($testimonial_info)) {
			$this->data['audio'] = basename($testimonial_info['audio']);
		} else {
			$this->data['audio'] = '';
		}
		...
		if (isset($this->request->post['video'])) {
			...
			$this->data['video'] = $testimonial_info['video'];
		} else {
			$this->data['video'] = '';
		}
	...
	private function validateForm() {
		...
		foreach ($this->request->post['testimonial_description'] as $language_id => $value) {
			if ((utf8_strlen($value['description']) < 10) || (utf8_strlen($value['description']) > 2000)) {
	...
	public function upload_audio() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && !isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				$file = uniqid('audio_').'.'.pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
										
	move_uploaded_file($this->request->files['file']['tmp_name'], DIR_IMAGE .'/review/audio/' . $file);
				
			$audio = 'image/review/audio/' . $file;
