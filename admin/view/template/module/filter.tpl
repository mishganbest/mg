...
<div class="box">
  ...
  <div class="content">
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
	  <table class="form">
        <tr>
          <td><?php echo $entry_count_enabled; ?></td>
          <td>
			<select name="count_enabled">		   
				   <option value="1" <?php if ($count_enabled) { ?>selected="selected"<?php } ?>><?php echo $text_enabled; ?></option>
				   <option value="0" <?php if (!$count_enabled) { ?>selected="selected"<?php } ?>><?php echo $text_disabled; ?></option>
			</select>
			&emsp;<span style="font-size:10px; color:#777;"><?php echo $info_count_enabled; ?></span>
		   </td>
        </tr>
	</table>
	  <table id="module" class="list">
        <thead>
          <tr>
			...
			<td class="left"><?php echo $entry_filter_group; ?></td>
        ...
        <tbody id="module-row<?php echo $module_row; ?>">
          <tr>
			...
			<td class="left"><select name="filter_module[<?php echo $module_row; ?>][filter_group_id]">
                <?php foreach ($filter_groups as $filter_group) { ?>
                <?php if ($filter_group['filter_group_id'] == $module['filter_group_id']) { ?>
                <option value="<?php echo $filter_group['filter_group_id']; ?>" selected="selected"><?php echo $filter_group['name']; ?></option>
                <?php } else { ?>
...
function addModule() {
	...
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="left"><select name="filter_module[' + module_row + '][filter_group_id]">';
	<?php foreach ($filter_groups as $filter_group) { ?>
	html += '      <option value="<?php echo $filter_group['filter_group_id']; ?>"><?php echo $filter_group['name']; ?></option>';
	<?php } ?>
	html += '    </select></td>';
