...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['name'])."', city = '".$this->db->escape($data['city'])."', phone = '".$this->db->escape($data['phone'])."', email = '".$this->db->escape($data['email'])."', vk = '".$this->db->escape($data['vk'])."', odnoklass = '".$this->db->escape($data['odnoklass'])."', image = '".$this->db->escape($data['image'])."', audio = '".$this->db->escape($data['audio'])."', video = '".$this->db->escape($data['video'])."', status = '" . (int)$data['status'] . "', rating = '".(int)$data['rating'] . "', date_added = '" . $this->db->escape($data['date_added']) . "'");

		$testimonial_id = $this->db->getLastId(); 
			
		foreach ($data['testimonial_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial_description SET testimonial_id = '" . (int)$testimonial_id . "', language_id = '" . (int)$language_id . "', description = '" . $this->db->escape($value['description']) . "'");
		}

	}
	
	public function editTestimonial($testimonial_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['name'])."', city = '".$this->db->escape($data['city'])."', phone = '".$this->db->escape($data['phone'])."', email = '".$this->db->escape($data['email'])."', vk = '".$this->db->escape($data['vk'])."', odnoklass = '".$this->db->escape($data['odnoklass'])."', image = '".$this->db->escape($data['image'])."', audio = '".$this->db->escape($data['audio'])."', video = '".$this->db->escape($data['video'])."', status = '" . (int)$data['status'] . "', date_added = '".$this->db->escape($data['date_added']). "', rating = '".(int)$data['rating']."' WHERE testimonial_id = '" . (int)$testimonial_id . "'");
	...
	public function getTestimonials($data = array()) {
		...
		if ($data) {
			...
			return $query->rows;
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "testimonial t LEFT JOIN " . DB_PREFIX . "testimonial_description td ON (t.testimonial_id = td.testimonial_id) WHERE td.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY t.name");
	...
	public function createDatabaseTables() {
		...
		$sql .= "`name` varchar(64) COLLATE utf8_general_ci NOT NULL, ";
		$sql .= "`city` varchar(64) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`phone` varchar(64) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`email` varchar(96) COLLATE utf8_general_ci DEFAULT NULL, ";
		$sql .= "`vk` varchar(128) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`odnoklass` varchar(128) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`image` varchar(128) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`audio` varchar(128) COLLATE utf8_general_ci DEFAULT NULL, "; 
		$sql .= "`video` varchar(128) COLLATE utf8_general_ci DEFAULT NULL, "; 
