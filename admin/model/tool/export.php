...
class ModelToolExport extends Model {
	...
	function storeCategoriesIntoDatabase( &$database, &$categories ) 
	{
		...
		foreach ($categories as $category) {
			...
			$table_tip = $category['table_tip'];
			$table_tip = ((strtoupper($table_tip)=="TRUE") || (strtoupper($table_tip)=="YES") || (strtoupper($table_tip)=="ENABLED")) ? 1 : 0;
			$table_poverhnost = $category['table_poverhnost'];
			$table_poverhnost = ((strtoupper($table_poverhnost)=="TRUE") || (strtoupper($table_poverhnost)=="YES") || (strtoupper($table_poverhnost)=="ENABLED")) ? 1 : 0;
			$table_size = $category['table_size'];
			$table_size = ((strtoupper($table_size)=="TRUE") || (strtoupper($table_size)=="YES") || (strtoupper($table_size)=="ENABLED")) ? 1 : 0;
			...
			$status = ((strtoupper($status)=="TRUE") || (strtoupper($status)=="YES") || (strtoupper($status)=="ENABLED")) ? 1 : 0;
			$sql2 = "INSERT INTO `".DB_PREFIX."category` (`category_id`, `image`, `parent_id`, `top`, `table_tip`, `table_poverhnost`, `table_size`, `column`, `sort_order`, `date_added`, `date_modified`, `status`) VALUES ";
			$sql2 .= "( $categoryId, '$imageName', $parentId, $top, $table_tip, $table_poverhnost, $table_size, $columns, $sortOrder, ";
	...
	function uploadCategories( &$reader, &$database ) 
	{
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$table_tip = $this->getCell($data,$i,$j++,($parentId=='0')?'true':'false');
			$table_poverhnost = $this->getCell($data,$i,$j++,($parentId=='0')?'true':'false');
			$table_size = $this->getCell($data,$i,$j++,($parentId=='0')?'true':'false');
			...
			$category['table_tip'] = $table_tip;
			$category['table_poverhnost'] = $table_poverhnost;
			$category['table_size'] = $table_size;
	...
	function validateCategories( &$reader )
	{
		...
		( "category_id", "parent_id", "name", "top", "table_tip", "table_poverhnost", "table_size", "columns", "sort_order", "image_name", "date_added", "date_modified", "language_id", "seo_keyword", "description", "meta_description", "meta_keywords", "seo_title", "seo_h1", "store_ids", "layout", "status\nenabled" );
		$data =& $reader->getSheet(0);
	...
	function populateCategoriesWorksheet( &$worksheet, &$database, $languageId, &$boxFormat, &$textFormat )
	{
		...
		$worksheet->setColumn($j,$j++,max(strlen('table_tip'),5)+1);
		$worksheet->setColumn($j,$j++,max(strlen('table_poverhnost'),5)+1);
		$worksheet->setColumn($j,$j++,max(strlen('table_size'),5)+1);
		...
		$worksheet->writeString( $i, $j++, 'table_tip', $boxFormat );
		$worksheet->writeString( $i, $j++, 'table_poverhnost', $boxFormat );
		$worksheet->writeString( $i, $j++, 'table_size', $boxFormat );
		...
		foreach ($result->rows as $row) {
			...
			$worksheet->write( $i, $j++, ($row['table_tip']==0) ? "false" : "true", $textFormat );
			$worksheet->write( $i, $j++, ($row['table_poverhnost']==0) ? "false" : "true", $textFormat );
			$worksheet->write( $i, $j++, ($row['table_size']==0) ? "false" : "true", $textFormat );
