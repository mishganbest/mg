...
class ModelToolExchange1c extends Model {
			...
			foreach ($query->rows as $orders_data) {
				...
				foreach ($products as $product) {
					...
					if (count($order_options) > 0)
					{
						...
						{
							$id = $this->get1CProductIdByProductId($product['product_id'], $order_option['product_option_value_id']);
						
							$document['Документ' . $document_counter]['Товары']['Товар' . $product_counter] = array(
								 'Ид'             => $id
								,'Наименование'   => $product['name'] . ' ' . $order_option['value']
					...
					{
						$id = $this->get1CProductIdByProductId($product['product_id'], 0);
		...
		if ($xml->ПакетПредложений->Предложения->Предложение) {
			foreach ($xml->ПакетПредложений->Предложения->Предложение as $offer) {
				...
				if (!$exchange1c_relatedoptions || $new_product) {
					...
					$product_option_value_id = $this->getProductOptionValueIdBy1CProductId($uuid[0]);
				...
				if (!$exchange1c_relatedoptions || $offer_cnt == count($xml->ПакетПредложений->Предложения->Предложение)
					|| $data['1c_id'] != substr($xml->ПакетПредложений->Предложения->Предложение[$offer_cnt]->Ид, 0, strlen($data['1c_id'])) ) {
						...
						$this->updateProduct($data, $product_id, $product_option_value_id, $language_id);
						unset($data);
				}
	...
	*/
	...
	private function setProduct($product, $language_id) {
		...
		if ($product_id) {
			$product_option_value_id = $this->getProductOptionValueIdBy1CProductId($product['1c_id']);
			
			$this->updateProduct($product, $product_id, $product_option_value_id, $language_id);
		}
		else {
			...
			if ($this->config->get('exchange1c_dont_use_artsync')) {
				...
				$product_option_value_id = $this->getProductOptionValueBySKU($data['sku']);
				if ($product_option_value_id != false)
				{
					$product_id = $this->getProductIdByProductOptionValueID($product_option_value_id);
				}
				else
				{
			...
			if ($product_id){
				...
				{
					$this->db->query('INSERT INTO `' .  DB_PREFIX . 'product_to_1c` SET product_id = ' . (int)$product_id . ', `product_option_value_id` = ' . (int)$product_option_value_id . ', `1c_id` = "' . $this->db->escape($product['1c_id']) . '"');
				}
				else
				{
	 ...
	 * @param int
	 */
	private function updateProduct($product, $product_id = false, $product_option_value_id = false, $language_id) {
		...
		{
			//Редактируем продукт
			$product_id = $this->model_catalog_product->editProductOptionValuePriceAndCount($product_id, $product_option_value_id, $product_old);
		}
		else
		{
			...
			//Редактируем продукт
			$product_id = $this->model_catalog_product->editProductPriceAndCount($product_id, $product_old);
		}
	...
	private function getProductOptionValueBySKU($sku) {
	...
	private function getProductIdByProductOptionValueID($product_option_value_id) {

		$query = $this->db->query('SELECT product_id FROM ' . DB_PREFIX . 'product_option_value WHERE `product_option_value_id` = ' . $product_option_value_id );
	 ...
	 * @return	string|bool
	 */
	private function get1CProductIdByProductId($product_id, $product_option_value_id) {

		$query0 = $this->db->query('SELECT 1c_id FROM ' . DB_PREFIX . 'product_to_1c WHERE `product_option_value_id` = ' . $product_option_value_id );
		if ($query0->num_rows) {
			return $query0->row['1c_id'];
		}
	...
	private function getProductOptionValueIdBy1CProductId($product_id) {

		$query = $this->db->query('SELECT product_option_value_id FROM ' . DB_PREFIX . 'product_to_1c WHERE `1c_id` = "' . $product_id . '"');
