...
class ModelSaleOrder extends Model {
	...
	public function getOrderTotalsShipping($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$order_id . "' and code = 'shipping' ORDER BY sort_order");

		return $query->rows;
	}

	public function getOrderTotalsInvoice($order_id) {
		$query = $this->db->query("SELECT text FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$order_id . "' and code = 'total' ORDER BY sort_order");
